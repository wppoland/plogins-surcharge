...
final class Settings implements HasHooks
{
    ...
    public function addMenuPage(): void
    {
        add_submenu_page(
            'woocommerce',
            __('Krompago: checkout fees', 'krompago'),
            __('Surcharge', 'krompago'),
            'manage_woocommerce',
            self::PAGE,
            [$this, 'renderPage'],
        );
    ...
    public function renderPage(): void
    {
        ...
        <div class="wrap surcharge-admin">
            <h1>
                ...
                <?php if ($enabled) : ?>
                    <span class="surcharge-admin__status surcharge-admin__status--on"><?php esc_html_e('Active', 'krompago'); ?></span>
                <?php else : ?>
                    <span class="surcharge-admin__status surcharge-admin__status--off"><?php esc_html_e('Disabled', 'krompago'); ?></span>
                <?php endif; ?>
            </h1>
            ...
            <div class="surcharge-admin__intro">
                ...
                <div>
                    <h2><?php esc_html_e('Add checkout fees in two steps', 'krompago'); ?></h2>
                    <p><?php esc_html_e('1. Add a fee row below. 2. Choose a fixed amount or a percentage of the cart. Fees appear in the cart, at checkout and on the order.', 'krompago'); ?></p>
                </div>
            </div>
            ...
            <form method="post" action="options.php">
                ...
                <div class="surcharge-admin__card">
                    <h2><?php esc_html_e('General', 'krompago'); ?></h2>
                    <table class="form-table" role="presentation">
                        <tbody>
                            <tr>
                                <th scope="row">
                                    <?php esc_html_e('Enable fees', 'krompago'); ?>
                                </th>
                                <td>
                                    <label for="surcharge_enabled">
                                        ...
                                        <?php esc_html_e('Apply the fees below at cart and checkout.', 'krompago'); ?>
                                    </label>
                                    <p class="description"><?php esc_html_e('Master switch. When off, no fees are added to any cart, regardless of the rows below.', 'krompago'); ?></p>
                                </td>
                            </tr>
                        </tbody>
                    </table>
                ...
                <div class="surcharge-admin__card">
                    <div class="surcharge-admin__card-head">
                        <h2><?php esc_html_e('Fees', 'krompago'); ?></h2>
                        <p class="surcharge-admin__card-hint"><?php esc_html_e('Each row is one fee. A fee with no label is skipped.', 'krompago'); ?></p>
                    </div>

                    <div class="surcharge-fees" id="surcharge-fees"
                        data-add-label="<?php esc_attr_e('Add fee', 'krompago'); ?>">
                        <?php
                        if (empty($fees)) {
                    ...
                    <p>
                        <button type="button" class="button surcharge-add-fee" id="surcharge-add-fee">
                            <span aria-hidden="true">+</span> <?php esc_html_e('Add fee', 'krompago'); ?>
                        </button>
                    </p>
    ...
    private function renderFeeRow(string $option, int $index, ?Fee $fee): void
    {
        ...
        <fieldset class="surcharge-fee" data-index="<?php echo esc_attr((string) $index); ?>">
            <legend class="screen-reader-text"><?php esc_html_e('Fee', 'krompago'); ?></legend>

            <div class="surcharge-fee__grid">
                <p class="surcharge-fee__field surcharge-fee__field--label">
                    <label for="<?php echo esc_attr($uid . '-label'); ?>"><?php esc_html_e('Label', 'krompago'); ?></label>
                    <input type="text" id="<?php echo esc_attr($uid . '-label'); ?>" name="<?php echo esc_attr($base . '[label]'); ?>" value="<?php echo esc_attr($label); ?>" class="regular-text" placeholder="<?php esc_attr_e('e.g. Handling fee', 'krompago'); ?>" aria-describedby="<?php echo esc_attr($uid . '-label-help'); ?>" />
                    <span id="<?php echo esc_attr($uid . '-label-help'); ?>" class="surcharge-fee__help"><?php esc_html_e('The wording customers see on the cart, checkout and order. Leave blank to skip this row.', 'krompago'); ?></span>
                </p>

                <p class="surcharge-fee__field">
                    <label for="<?php echo esc_attr($uid . '-type'); ?>"><?php esc_html_e('Type', 'krompago'); ?></label>
                    <select id="<?php echo esc_attr($uid . '-type'); ?>" name="<?php echo esc_attr($base . '[type]'); ?>" class="surcharge-fee__type" aria-describedby="<?php echo esc_attr($uid . '-type-help'); ?>">
                        <option value="fixed" <?php selected($type, Fee::TYPE_FIXED); ?>><?php esc_html_e('Fixed amount', 'krompago'); ?></option>
                        <option value="percent" <?php selected($type, Fee::TYPE_PERCENT); ?>><?php esc_html_e('Percentage of cart', 'krompago'); ?></option>
                    </select>
                    <span id="<?php echo esc_attr($uid . '-type-help'); ?>" class="surcharge-fee__help"><?php esc_html_e('Fixed adds the same amount every order. Percentage scales with the cart subtotal.', 'krompago'); ?></span>
                </p>

                <p class="surcharge-fee__field">
                    <label for="<?php echo esc_attr($uid . '-amount'); ?>"><?php esc_html_e('Amount', 'krompago'); ?></label>
                    ...
                    <span id="<?php echo esc_attr($uid . '-amount-help'); ?>" class="surcharge-fee__help">
                        <span class="surcharge-fee__help-for-fixed"><?php esc_html_e('Charged in your store currency, e.g. 3.50.', 'krompago'); ?></span>
                        <span class="surcharge-fee__help-for-percent"><?php esc_html_e('A percentage from 0 to 100, e.g. 2.5 adds 2.5% of the subtotal.', 'krompago'); ?></span>
                    </span>
                </p>
                ...
                <p class="surcharge-fee__field surcharge-fee__field--readout" aria-hidden="true">
                    <span class="surcharge-fee__readout" data-fixed-glyph="&#43;&#36;" data-percent-glyph="&#43;&#37;">
                        ...
                        <span class="surcharge-fee__readout-text"><?php esc_html_e('Receipt line', 'krompago'); ?></span>
                    </span>
                </p>
                ...
                <p class="surcharge-fee__field surcharge-fee__field--check">
                    <label for="<?php echo esc_attr($uid . '-taxable'); ?>">
                        ...
                        <?php esc_html_e('Taxable', 'krompago'); ?>
                    </label>
                    <span id="<?php echo esc_attr($uid . '-taxable-help'); ?>" class="surcharge-fee__help"><?php esc_html_e('On: tax is calculated on this fee using your standard rate. Off: the fee is added tax-free.', 'krompago'); ?></span>
                </p>
            </div>
            ...
            <div class="surcharge-fee__footer">
                <label class="surcharge-fee__enabled" for="<?php echo esc_attr($uid . '-enabled'); ?>">
                    ...
                    <?php esc_html_e('Enabled', 'krompago'); ?>
                    <span id="<?php echo esc_attr($uid . '-enabled-help'); ?>" class="surcharge-fee__help surcharge-fee__help--inline"><?php esc_html_e('Off keeps the row saved but stops charging it.', 'krompago'); ?></span>
                </label>
                <button type="button" class="button-link surcharge-fee__remove"><?php esc_html_e('Remove fee', 'krompago'); ?></button>
            </div>
        </fieldset>
        <?php
    }
